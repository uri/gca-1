...
namespace WindowsPhoneGame2
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        public void gameStart(GameTime gameTime){
            timeOfGameStart = gameTime.ElapsedGameTime.TotalMilliseconds;
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if(!started && state == 4){
                timeOfGameStart = gameTime.TotalGameTime.TotalMilliseconds;
                started = true;
            }

            if(gameTime.TotalGameTime.TotalMilliseconds - timeOfGameStart > 30000 && state == 4){
                ended = true;
            }
            ...
            if (!ended)
            {
                ...
                if (rand.NextDouble() < 0.0025 * scrollingSpeed)
                {
                ...
                for (int i = 0; i < obstacles.Count; i++)
                {
                    ...
                    if (ob.position.X < 0)
                    {
                        ...
                        {
                            if (state == 4)
                                numberSystem.RegisterString(obstacles[0].position, gameTime, -17453, this);
        ...
        Vector2 otherFontPos = new Vector2(745, 0);
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            if (!ended)
            {
                ...
                if (state == 4)
                {
                    if ((30 - ((gameTime.TotalGameTime.TotalMilliseconds - timeOfGameStart) / 1000.0f)) > 10)
                    {
                        spriteBatch.DrawString(font, "" + (int)(30 - ((gameTime.TotalGameTime.TotalMilliseconds - timeOfGameStart) / 1000.0f)), otherFontPos, Color.Green);
                    }
                    else
                    {
                        if ((int)((gameTime.TotalGameTime.TotalMilliseconds - timeOfGameStart) / 1000.0f) % 2 == 0)
                        {
                            spriteBatch.DrawString(font, "" + (int)(30 - ((gameTime.TotalGameTime.TotalMilliseconds - timeOfGameStart) / 1000.0f)), otherFontPos, Color.Red);
                        }
                        else
                        {
                            spriteBatch.DrawString(font, "" + (int)(30 - ((gameTime.TotalGameTime.TotalMilliseconds - timeOfGameStart) / 1000.0f)), otherFontPos, Color.Yellow);
                        }
                    }
             ...
             else // Game not in game
            {
                spriteBatch.DrawString(font, "Congrats " , fontPos, Color.Black);

                Vector2 newVec = new Vector2(fontPos.X, fontPos.Y + 40);
                spriteBatch.DrawString(font, "Score : " + score, newVec, Color.Black);

                Vector2 newVecw = new Vector2(fontPos.X, newVec.Y + 40);
                spriteBatch.DrawString(font, "Tap to play again! " + score, newVecw, Color.Black);
        ...
        void HandleGestures2(GameTime gameTime)
        {
